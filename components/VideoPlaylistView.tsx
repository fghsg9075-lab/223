...
export const VideoPlaylistView: React.FC<Props> = ({ 
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-20 animate-in fade-in slide-in-from-right-8">
       ...
       {/* PLAYER AREA */}
       {activeVideo ? (
           <div ref={playerContainerRef} className="aspect-video bg-black w-full sticky top-[73px] z-10 relative group">
               ...
               {/* FULL SCREEN TOGGLE (z-50 to stay above blockers) */}
               <button 
                   onClick={toggleFullScreen}
                   className="absolute top-2 right-2 z-50 bg-black/50 p-2 rounded-full text-white/80 hover:text-white hover:bg-black/70 backdrop-blur-sm transition-all shadow-lg"
                   title="Full Screen"
               >
                   <Maximize size={18} />
               </button>
